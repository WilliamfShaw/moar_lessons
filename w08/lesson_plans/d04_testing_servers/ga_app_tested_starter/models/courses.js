...
module.exports = function(sequelize, DataTypes) {
	...
	var courses = sequelize.define('courses', {
		...
		},
		{
			...
			instanceMethods: {
				hours: function() {
					if (this.category === 'Immersive') {
						return this.length * 40;
					}

					if (this.category === 'Part-Time') {
			...
			classMethods: {
				emptyCourses: function(models, callback) {
					courses
						.findAll({ include: models.students })
						.then(function(foundCourses) {
							var emptyCourses = foundCourses.filter(function(course) {
				        return course.students.length === 0;
				      });

							callback(emptyCourses);
						});
				},
